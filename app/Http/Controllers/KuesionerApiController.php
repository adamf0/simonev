...
class KuesionerApiController extends Controller
{
    ...
    public function listKuesioner(Request $request)
    {
        ...
        $results2 = $bank_soal->get()
                    ...
                    ->filter(fn($item)=> date('Y') || strtotime(now()) >= strtotime($item->start_repair." 00:00:00") && strtotime(now()) <= strtotime($item->end_repair." 23:59:59"))
                    ->values();

        dd($results2,$bank_soal->get(),$bank_soal->toRawSql());
        $perPage = 5;
